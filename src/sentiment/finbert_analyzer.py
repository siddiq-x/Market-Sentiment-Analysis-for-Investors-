...
class FinBERTAnalyzer:
    ...
    def _apply_financial_context(self, 
        ...
        strong_positive_count = sum(1 for word in self.financial_modifiers['strong_positive'] 
                                  if word in text_lower)
        strong_negative_count = sum(1 for word in self.financial_modifiers['strong_negative'] 
                                  if word in text_lower)
        uncertainty_count = sum(1 for word in self.financial_modifiers['uncertainty'] 
                              if word in text_lower)
        ...
        if strong_positive_count > 0:
            ...
            adjusted_scores["negative"] = max(0.0, adjusted_scores["negative"] - boost/2)
        ...
        if strong_negative_count > 0:
            ...
            adjusted_scores["positive"] = max(0.0, adjusted_scores["positive"] - boost/2)
    ...
    def batch_analyze(self, 
                     texts: List[str], 
                     contexts: Optional[List[Dict[str, Any]]] = None) -> List[SentimentResult]:
