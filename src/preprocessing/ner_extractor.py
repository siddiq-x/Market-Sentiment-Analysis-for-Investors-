...
class FinancialNER:
    ...
    def _add_financial_patterns(self):
        ...
        metric_patterns = [
            [{"LOWER": {"IN": ["eps", "pe", "pb", "roe", "roa", "ebitda", "revenue", "profit", "margin"]}}],
        ...
        # Market events
        event_patterns = [
            [{"LOWER": {"IN": ["ipo", "merger", "acquisition", "buyout", "spinoff"]}}],
            [{"LOWER": "earnings"}, {"LOWER": "report"}],
            [{"LOWER": "dividend"}, {"LOWER": "announcement"}],
            [{"LOWER": "stock"}, {"LOWER": "split"}],
            [{"LOWER": "market"}, {"LOWER": {"IN": ["crash", "rally", "correction", "bubble"]}}]
        ]
        self.matcher.add("MARKET_EVENT", event_patterns)
        
        # Financial instruments
        instrument_patterns = [
            [{"LOWER": {"IN": ["stock", "bond", "option", "future", "etf", "mutual", "reit"]}}],
            [{"LOWER": "call"}, {"LOWER": "option"}],
            [{"LOWER": "put"}, {"LOWER": "option"}],
            [{"LOWER": "treasury"}, {"LOWER": {"IN": ["bill", "note", "bond"]}}]
        ]
        self.matcher.add("FINANCIAL_INSTRUMENT", instrument_patterns)
        
        # Analyst ratings
        rating_patterns = [
            [{"LOWER": {"IN": ["buy", "sell", "hold", "strong", "weak", "outperform", "underperform"]}}],
            [{"LOWER": "price"}, {"LOWER": "target"}],
            [{"LOWER": {"IN": ["upgrade", "downgrade", "initiate", "coverage"]}}]
        ]
    ...
    def _load_financial_entities(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'sectors': [
                ...
                'Biotechnology', 'Pharmaceuticals', 'Banking', 'Insurance', 'Oil & Gas'
            ],
    ...
    def extract_entities(self, text: str, include_context: bool = True) -> List[FinancialEntity]:
        ...
        for match_id, start, end in matches:
            ...
            # Avoid duplicates
            if not any(e.start <= span.start_char < e.end or e.start < span.end_char <= e.end for e in entities):
                context = self._get_context(text, span.start_char, span.end_char) if include_context else ""
        ...
        additional_entities = self._extract_additional_entities(text, include_context)
    ...
    def _is_financial_entity(self, ent) -> bool:
        ...
        financial_labels = {'ORG', 'MONEY', 'PERCENT', 'CARDINAL', 'DATE', 'GPE'}
    ...
    def _is_valid_ticker(self, ticker: str) -> bool:
        ...
        false_positives = {
            ...
            'AT', 'OR', 'SO', 'AN', 'GO', 'US', 'AM', 'GET', 'NEW', 'NOW', 'WAY'
        }
    ...
    def get_entity_summary(self, entities: List[FinancialEntity]) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_type": max(entity_counts.items(), key=lambda x: x[1])[0] if entity_counts else None,
    ...
    def extract_ticker_mentions(self, text: str) -> List[Tuple[str, int, str]]:
        ...
        for ticker, count in ticker_counts.items():
            ...
            best_context = max(ticker_contexts[ticker], key=len) if ticker_contexts[ticker] else ""
