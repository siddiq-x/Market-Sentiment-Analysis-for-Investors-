...
def get_sentiment_data(ticker):
    ...
    try:
        ...
        return jsonify(
            {"ticker": ticker, "data": processed_data, "count": len(processed_data)}
        )
...
def get_market_data(ticker):
    ...
    try:
        ...
        market_data = ingestion_manager.fetch_by_connector(
            "market", tickers=[ticker], period="1d", interval="1h"
        )
...
def get_sentiment_chart(ticker):
    ...
    try:
        ...
        fig.update_layout(
            ...
            yaxis2=dict(title="Confidence", side="right", overlaying="y", range=[0, 1]),
            hovermode="x unified",
    ...
    try:
        if dashboard_state["stream_processor_running"]:
            ...
            return jsonify(
                {
                    "status": "stopped",
                    "message": "Stream processor stopped successfully",
                }
            )
        else:
            return jsonify(
                {"status": "not_running", "message": "Stream processor is not running"}
            )
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
            ...
            processed_results.append(
                {
                    ...
                    "sentiment": (
                        result.sentiment_result.get("sentiment")
                        if result.sentiment_result
                        else None
                    ),
                    "confidence": (
                        result.sentiment_result.get("confidence")
                        if result.sentiment_result
                        else None
                    ),
...
def handle_subscribe_ticker(data):
    ...
    if ticker in dashboard_state["active_tickers"]:
        ...
        emit(
            "subscribed",
            {"ticker": ticker, "message": f"Subscribed to {ticker} updates"},
        )
