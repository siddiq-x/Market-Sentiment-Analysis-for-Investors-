...
class FusionManager:
    ...
    def _sentiment_to_dataframe(
        ...
        for result in sentiment_results:
            ...
            if result.finbert_result and (
                not result.finbert_result.metadata.get("ticker")
                or result.finbert_result.metadata.get("ticker") == target_ticker
            ):
                ...
                data.append(
                    {
                        ...
                        "credibility_score": (
                            getattr(result.finbert_result, "credibility_score", 1.0)
    ...
    def _market_data_to_dataframe(
        self, market_data_points: List[DataPoint]
    ) -> pd.DataFrame:
    ...
    def _analyze_contributing_factors(
        ...
        recent_market = [
            p
            for p in market_data_points[-10:]
            if p.metadata.get("data_type") == "stock_price"
        ]
    ...
    def export_predictions(
        self, predictions: Dict[str, FusionPrediction]
    ) -> pd.DataFrame:
            ...
            data.append(
                {
                    ...
                    "prediction_label": ["bearish", "neutral", "bullish"][
