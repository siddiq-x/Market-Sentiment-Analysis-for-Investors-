...
class MultimodalFeatureEngineer:
    ...
    def create_features(self, 
        ...
        metadata = {
            ...
            "feature_count": feature_sequences.shape[-1] if len(feature_sequences.shape) > 1 else 0,
            "sequence_length": feature_sequences.shape[1] if len(feature_sequences.shape) > 2 else 0,
    ...
    def _align_data(self, 
        ...
        try:
            ...
            # Merge on timestamp
            aligned = pd.merge(sentiment_hourly, market_hourly, on='timestamp', how='inner')
    ...
    def _create_technical_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(data) > 20:
            ...
            technical_features['bb_position'] = (close_prices - technical_features['bb_lower']) / (
    ...
    def _create_sequences(self, 
                         ...
                         data: pd.DataFrame,
                         target_ticker: str) -> Tuple[np.ndarray, np.ndarray, List[datetime]]:
        ...
        for i in range(self.lookback_window, len(feature_array) - self.prediction_horizon):
            # Feature sequence
    ...
    def _normalize_features(self, features: np.ndarray) -> np.ndarray:
        ...
        if not self.scalers_fitted:
            ...
            self.sentiment_scaler.fit(flattened[:, :10])  # First 10 features assumed to be sentiment
            self.market_scaler.fit(flattened[:, 10:])     # Rest assumed to be market
