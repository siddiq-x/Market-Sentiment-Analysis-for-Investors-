...
class MultimodalFeatureEngineer:
    ...
    def create_features(
        ...
        if self.selector_fitted:
            ...
            ).reshape(feature_sequences.shape[0], -1, feature_sequences.shape[2])
        ...
        metadata = {
            ...
            "feature_count": (
                feature_sequences.shape[-1] if len(feature_sequences.shape) > 1 else 0
            ),
            "sequence_length": (
                feature_sequences.shape[1] if len(feature_sequences.shape) > 2 else 0
            ),
    ...
    def _align_data(
        ...
        try:
            ...
            ticker_market_data = market_data[
                market_data.get("ticker", "") == target_ticker
            ].copy()
            ...
            # Merge on timestamp
            aligned = pd.merge(
                sentiment_hourly, market_hourly, on="timestamp", how="inner"
            )
    ...
    def _create_time_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        time_features["hour_sin"] = np.sin(2 * np.pi * df["timestamp"].dt.hour / 24)
        time_features["hour_cos"] = np.cos(2 * np.pi * df["timestamp"].dt.hour / 24)

        # Day of week (cyclical encoding)
        time_features["dow_sin"] = np.sin(2 * np.pi * df["timestamp"].dt.dayofweek / 7)
        time_features["dow_cos"] = np.cos(2 * np.pi * df["timestamp"].dt.dayofweek / 7)

        # Month (cyclical encoding)
        time_features["month_sin"] = np.sin(2 * np.pi * df["timestamp"].dt.month / 12)
        time_features["month_cos"] = np.cos(2 * np.pi * df["timestamp"].dt.month / 12)
    ...
    def _create_sentiment_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [3, 6, 12, 24]:
            if len(data) > window:
                ...
                sentiment_features[f"sentiment_momentum_{window}h"] = data[
                    "sentiment_score"
        ...
        sentiment_features["sentiment_consensus"] = np.abs(
    ...
    def _create_market_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        market_features["volume_ratio"] = (
            data["volume"] / market_features["volume_ma_24h"]
        )
    ...
    def _create_technical_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(data) > 20:
            ...
            technical_features["bb_position"] = (
                close_prices - technical_features["bb_lower"]
    ...
    def _create_interaction_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        interaction_features["sentiment_volume_interaction"] = (
            sentiment_score * np.log1p(volume)
        )
    ...
    def _create_sequences(
        self, features: pd.DataFrame, data: pd.DataFrame, target_ticker: str
        ...
        for i in range(
            self.lookback_window, len(feature_array) - self.prediction_horizon
        ):
            # Feature sequence
    ...
    def _create_target(self, data: pd.DataFrame) -> pd.Series:
        ...
        future_returns = close_prices.shift(-self.prediction_horizon).pct_change(
    ...
    def _normalize_features(self, features: np.ndarray) -> np.ndarray:
        ...
        for seq in features:
            ...
            normalized_seq[:, :10] = self.sentiment_scaler.transform(seq[:, :10])
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        if self.selector_fitted:
            ...
            importance_dict = {}
            for i, (selected, score) in enumerate(zip(selected_features, scores)):
                if selected:
