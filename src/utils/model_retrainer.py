...
class ModelRetrainer:
    ...
    def _check_immediate_retraining(self, ticker: str):
        ...
        recent_predictions = self.performance_history[ticker][-20:]  # Last 20 predictions
        ...
        if len(recent_predictions) >= 10:
            ...
            if accuracy < self.performance_threshold:
                self.logger.warning(f"Poor performance detected for {ticker}: {accuracy:.3f}")
    ...
    def _evaluate_model_performance(self, metrics: ModelPerformanceMetrics):
        ...
        self.logger.info(f"Model performance for {ticker}: Accuracy={metrics.accuracy:.3f}, F1={metrics.f1_score:.3f}")
        ...
        for ticker in config.monitored_tickers:
            try:
                ...
                if len(recent_predictions) >= 5 and len(historical_predictions) >= 20:
                    ...
                    historical_accuracy = sum(1 for p in historical_predictions if p['correct']) / len(historical_predictions)
    ...
    def _execute_retraining_job(self, job: RetrainingJob):
        ...
        try:
            ...
            if not training_data or len(training_data) < self.min_data_points:
                self.logger.warning(f"Insufficient data for retraining {ticker}: {len(training_data) if training_data else 0} points")
                return
        ...
        return {
            ...
            "performance_summary": {
                ticker: {
                    ...
                    "recent_accuracy": sum(1 for p in history[-50:] if p['correct']) / min(50, len(history)) if history else 0
                }
