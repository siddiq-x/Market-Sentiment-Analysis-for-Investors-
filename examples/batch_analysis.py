...
def batch_sentiment_analysis(tickers, days_back=7):
    ...
    """
    print(f"Starting batch analysis for {len(tickers)} tickers over "
          f"{days_back} days")
...
def generate_report(results, output_file=None):
    ...
    for ticker, data in results.items():
        ...
        report.append(
            f"  Overall Sentiment: {data['overall_sentiment_score']:.3f}"
        )
        report.append(
            f"  Confidence: {data['sentiment_confidence']:.3f}"
        )
        ...
        for ticker, data in results.items():
            ...
            if 'daily_sentiment' in json_data:
                for day, day_data in json_data['daily_sentiment'].items():
                    day_data['scores'] = [
                        float(score) for score in day_data['scores']
                    ]
