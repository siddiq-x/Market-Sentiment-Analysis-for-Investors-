...
def batch_sentiment_analysis(tickers, days_back=7):
    ...
    """
    print(f"Starting batch analysis for {len(tickers)} tickers over {days_back} days")
    ...
    for ticker in tickers:
        ...
        try:
            # Fetch data
            data_points = ingestion_manager.fetch_all_data([ticker], hours_back=days_back * 24)
            ...
            for dp in data_points:
                ...
                if dp.source in ['twitter', 'reddit']:
                    bot_score = bot_detector.detect_bot(dp.content, {
                ...
                sentiment_result = sentiment_analyzer.analyze_sentiment(
                    cleaned_text, 
                    context={'source': dp.source, 'ticker': ticker}
                )
                ...
                for entity_type, entity_list in entities.items():
                    ...
                    for entity in entity_list:
                        if entity not in ticker_results['entities'][entity_type]:
                            ticker_results['entities'][entity_type][entity] = 0
                        ticker_results['entities'][entity_type][entity] += 1
                ...
                if day_key not in ticker_results['daily_sentiment']:
                    ticker_results['daily_sentiment'][day_key] = {
                        'scores': [], 'count': 0, 'avg_score': 0
                    }
                ticker_results['daily_sentiment'][day_key]['scores'].append(sentiment_result.ensemble_score)
            ...
            if total_count > 0:
                ticker_results['overall_sentiment_score'] = (
                    (total_sentiment['bullish'] - total_sentiment['bearish']) / total_count
                )
                ticker_results['sentiment_confidence'] = max(total_sentiment.values()) / total_count
            else:
            ...
            print(f"Processed {ticker}: {total_count} data points, "
...
def generate_report(results, output_file=None):
    ...
    total_data_points = sum(r.get('total_data_points', 0) for r in results.values() if 'error' not in r)
    avg_sentiment = sum(r.get('overall_sentiment_score', 0) for r in results.values() if 'error' not in r) / len([r for r in results.values() if 'error' not in r]) if results else 0
    ...
    for ticker, data in results.items():
        ...
        if total > 0:
            ...
            report.append(f"    Bullish: {sentiment_dist['bullish']} ({sentiment_dist['bullish']/total*100:.1f}%)")
            report.append(f"    Neutral: {sentiment_dist['neutral']} ({sentiment_dist['neutral']/total*100:.1f}%)")
            report.append(f"    Bearish: {sentiment_dist['bearish']} ({sentiment_dist['bearish']/total*100:.1f}%)")
        
        # Source breakdown
        if data['source_breakdown']:
            report.append(f"  Top Sources:")
            sorted_sources = sorted(data['source_breakdown'].items(), key=lambda x: x[1], reverse=True)
            for source, count in sorted_sources[:3]:
                report.append(f"    {source}: {count}")
        
        # Top entities
        if data['entities'].get('ORGANIZATION'):
            top_orgs = sorted(data['entities']['ORGANIZATION'].items(), key=lambda x: x[1], reverse=True)[:3]
            report.append(f"  Top Organizations: {', '.join([org for org, _ in top_orgs])}")
