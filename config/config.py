...
class Config:
    ...
    def validate(self) -> List[str]:
        ...
        if not self.api.news_api_key:
            missing.append("NEWS_API_KEY - Required for fetching news from newsapi.org")
        if not self.api.twitter_bearer_token:
            missing.append("TWITTER_BEARER_TOKEN - Required for fetching tweets")
        if not self.api.alpha_vantage_key:
            missing.append("ALPHA_VANTAGE_API_KEY - Required for fetching market data")
        if not self.api.fred_api_key:
            missing.append("FRED_API_KEY - Required for fetching economic indicators")
